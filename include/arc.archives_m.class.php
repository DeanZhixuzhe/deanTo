...
class Archives
{
    ...
    function ParAddTable()
    {
        ...
        if($this->ChannelUnit->ChannelInfos['addtable']!='')
        {
            ...
            $this->Fields['bodysi'] = preg_replace("@ [\s]{0,}alt[\s]{0,}=[\"'\s]{0,}[\s\S]{0,}[\"'\s] @isU"," ",$this->Fields['bodysi']);    
            $this->Fields['bodysi'] = str_ireplace("<img " ,"<img alt=\"".$this->Fields['title']."\" class=\"lazy_load\" ",$this->Fields['bodysi']); 
        }
        //完成附加表信息读取
        unset($row);
