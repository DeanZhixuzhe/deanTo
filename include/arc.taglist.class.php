...
class TagList
{
    ...
    function __construct($keyword, $templet)
    {
        ...
        if($this->Tag!='')
        {
            ...
            if (is_numeric($this->Tag)) {
                ...
                $this->Fields['title'] = $this->TagInfos['tag'];    //修正title调用TAG名称错误问题
            }else{
                $this->TagInfos = $this->dsql->GetOne("Select * From `dede_tagindex` where tagdir ='{$this->Tag}' ");
                $this->Fields['title'] = $this->TagInfos['tag'];    //修正title调用TAG名称错误问题
            }
    ...
    function GetPageListDM($list_len,$listitem="info,index,end,pre,next,pageno")
    {
        ...
        {
            if ($this->PageNo == 2) {
                $prepage.="<li><a href='".$purl."'>上一页</a></li>\r\n";
            }else{
    ...
    function GetCurUrl()
    {
        if(!empty($_SERVER["REQUEST_URI"]))
        {
            ...
            if (strstr($nowurl,".html")) {
                for ($i=0; $i < count($nowurls)-1; $i++) { 
                    $url .= $nowurls[$i]."/";
                }
                $nowurl = $url;
            }
