...
if($tid==0 && $channelid==0 && empty($_GET['tid'])) die(" Request Error! ");//修改的
...
{
    if(empty($tid)&&!empty($_GET['tid'])){//这一整段是修改的
        $tname=$_GET['tid'];
        $tinfos = $dsql->GetOne("SELECT ch.issystem FROM `#@__arctype` tp LEFT JOIN `#@__channeltype` ch ON ch.id=tp.channeltype WHERE tp.typedir LIKE '{cmspath}/$tname'");
    }else{
...
if($tinfos['issystem']==-1)
{
    ...
    if($tid==0){//这一整段是修改的
        $dsql;
        $tname1=$_GET['tid'];
        $selquery = "SELECT id,topid FROM `#@__arctype` WHERE typedir LIKE '{cmspath}/$tname1'";
        $dsql = &$dsql;
        $result=$dsql->SetQuery ( $selquery );
        $result=$dsql->Execute ();
        $arr = $dsql->GetArray ();
        $lv = new ListView($arr['id']);
    }else{
        $lv = new ListView($tid);
    }
    //对设置了会员级别的栏目进行处理
