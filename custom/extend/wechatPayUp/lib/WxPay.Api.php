...
class WxPayApi
{
	...
	public static function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		//获取沙箱密钥
		$sandUrl = "https://api.mch.weixin.qq.com/sandboxnew/pay/getsignkey";
		$sandArr = array();
		$sandArr['mch_id'] = WxPayConfig::MCHID;
		$sandArr['nonce_str'] = $inputObj->GetNonce_str();
		$sandArr['sign'] = $inputObj->GetSign();
		// $xml2 = self::getXml($sandArr);
		$xml2 = "<xml>";
    	foreach ($sandArr as $key=>$val)
    	{
    		if (is_numeric($val)){
    			$xml2.="<".$key.">".$val."</".$key.">";
    		}else{
    			$xml2.="<".$key."><![CDATA[".$val."]]></".$key.">";
    		}
        }
        $xml2.="</xml>";
        // return 'Ceshi'.$xml2;
		$sandResult = self::postXmlCurl($xml2, $sandUrl, false);
		return $sandResult;
		$inputObj->SetSandbox_signkey($sandResult['sandbox_signkey']);
	...
	private static function getXml($value)
	{
